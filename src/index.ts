...
export type {
  SkinManifestV4, SkinRuntimeCompatibility, SkinVisualsV1, VisualSlotId, ThemeLayerV2,
...
export async function apply(ctx: Context): Promise<void> {
  ...
  if (dshHomePath === undefined || webServer === undefined) {
    throw new Error('dsh-skin-plugin 0.4.0 requires DSH 0.1.0-rc.5 Host services (dshHomePath and webServer)')
  }
  const library = await SkinLibrary.open(
    dshHomePath('skins-v4'),
